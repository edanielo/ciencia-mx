...
def main():
    ...
    with outpath.open("w", encoding="utf-8") as out:
        for r in load_enriched(args.ri):
            ...
            # Opcionales del enriquecido (solo si traen valor)
            ...
            # --- Inyección Dublin Core (si existe mapeo sha256 -> xml_path -> dc) ---
            xml_path = fetch_map.get(sha)
            if xml_path:
                dc = dc_index.get(xml_path)
                if dc:
                    # title
                    titles = [
                        t
                    ...
                    # url / doi (solo si no vienen del enriquecido)
                    if "url_s" not in doc:
                        url_dc = dc.get("url")
